...
const ProductsData = ({ item }:ItemProps) => {
  ...
  return <div className='w-full rounded-lg overflow-hidden '>
    ...
    <div>
       ...
       {
        item?.isNew && <span className='absolute top-2 right-2 font-medium text-xs py-1 px-3 rounded-full bg-white group-hover:bg-orange-600 group-hover:text-white duration-200'> New Arrival</span>
       }
       </div>
  ...
  <button className='bg-orange-600 px-4 py-2 text-sm tracking-wide rounded-full text-slate-100 hover:bg-orange-800 hover:text-white duration-200'>add to cart</button>
   {/* star icons */}
   <div className='flex item-center gap-x-1'>{startArray}</div>
    </div>
    </div>
    </div>
  </div>
